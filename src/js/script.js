    ...
    Object.entries(grouped).forEach(([threshold, els]) => {
      const observer = new IntersectionObserver(entries => {
        entries.forEach(entry => {
          const targets = document.querySelectorAll(entry.target.getAttribute('data-inview-target')) || [];
          const isIntersecting = entry.isIntersecting;
          const isPassed = (entry.boundingClientRect.y <= 0 && !isIntersecting);

          [entry.target, ...targets].forEach(t => {
            ['passed', 'inview', 'notinview'].forEach(c => t.classList.toggle(c, c === 'passed' ? isPassed : c === 'inview' ? isIntersecting : !isIntersecting));
          });
        });
  ...
  document.querySelectorAll('.dual-range-slider-wrapper').forEach(slider => {
    ...
    const updateStyles = () => {
      ...
      toSlider.style.borderLeftColor = fromPercent == 0 ? 'var(--vs-black)' : 'var(--light-grey)';
      toSlider.style.borderRightColor = toPercent == 100 ? 'var(--vs-black)' : 'var(--light-grey)';
    };
    ...
    const updateActiveInput = (e) => {
      ...
      curFrom = clamp(isRange ? fromSlider.value : fromInput.value, minVal, curTo);
    ...
    const init = () => {
      [fromSlider, toSlider, fromInput, toInput].forEach(el => {
        el.min = minVal; el.max = maxVal; el.step = 1;
        if (el.type == 'range') {
